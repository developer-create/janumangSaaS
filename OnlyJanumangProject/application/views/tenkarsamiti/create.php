            ...
            <div class="col-md-12">
                <div class="box box-primary">
                    <div class="box-header">
                        <h3 class="box-title">Add New तेंकर समिति (Tenkar Samiti)</h3>
                    </div>
                    <form action="<?php echo site_url('tenkarsamiti/store'); ?>" method="post" enctype="multipart/form-data">
                        <div class="box-body">
                            <div class="row">
                                <div class="col-md-3">
                                    <div class="form-group">
                                        <label for="unique_id_preview">Unique ID:</label>
                                        <input type="text" class="form-control" id="unique_id_preview" value="Auto Generated (TS/X)" readonly style="background-color: #f5f5f5; color: #999;">
                        ...
                        <div class="box-footer">
                            ...
                            <a href="<?php echo site_url('tenkarsamiti'); ?>" class="btn btn-default">Cancel</a>
                        </div>
                    </form>
                </div>
            </div>
...
$(document).ready(function() {
    // Fetch next Unique ID preview via AJAX
    $.ajax({
        url: '<?php echo base_url(); ?>tenkarsamiti/next_unique_id',
        type: 'GET',
        dataType: 'json',
        ...
        error: function() {
            console.warn('Could not fetch next unique id preview');
        }
    });
    ...
    $('#block').change(function() {
        ...
        if (blockId) {
            $.ajax({
                url: '<?php echo base_url(); ?>tenkarsamiti/get_booths_by_block',
                type: 'POST',
    ...
    $('#booth_name').change(function() {
        ...
        if (boothId) {
            $.ajax({
                url: '<?php echo base_url(); ?>tenkarsamiti/get_booth_details',
                type: 'POST',
