            ...
            <div class="col-md-12">
                <div class="box box-primary">
                    <div class="box-header">
                        <h3 class="box-title">Edit ब्लॉक समिति (Block Samiti) Details</h3>
                    </div>
                    <form action="<?php echo site_url('blocksamiti/update/'.$record['id']); ?>" method="post" enctype="multipart/form-data">
                        <div class="box-body">
                            <div class="row">
                                <div class="col-md-3">
                        ...
                        <div class="box-footer">
                            ...
                            <a href="<?php echo site_url('blocksamiti'); ?>" class="btn btn-default">Cancel</a>
                        </div>
                    </form>
                </div>
            </div>
...
$(document).ready(function() {
    // Debug output to check PHP data
    console.log('Record data:', <?php echo json_encode($record); ?>);
    console.log('Booths data:', <?php echo json_encode($booths); ?>);
    console.log('Villages data:', <?php echo json_encode($villages); ?>);
    console.log('Selected booth_id from record:', '<?php echo isset($record["booth_id"]) ? $record["booth_id"] : "none"; ?>');
    console.log('Selected village from record:', '<?php echo isset($record["village"]) ? $record["village"] : "none"; ?>');
    
    console.log('Page loaded. Booth selected:', $('#booth_name').val(), 'Village selected:', $('#village').val());
    ...
    $('#block').change(function() {
        ...
        if (blockId) {
            $.ajax({
                url: '<?php echo base_url(); ?>blocksamiti/get_booths_by_block',
                type: 'POST',
                data: {block_id: blockId},
                dataType: 'json',
                success: function(response) {
                    if (!response.error && response.booths && response.booths.length > 0) {
                ...
                error: function(xhr, status, error) {
                    console.error('AJAX Error loading booths:', status, error);
                }
            });
        }
    });
    ...
    $('#booth_name').change(function() {
        ...
        if (boothId) {
            $.ajax({
                url: '<?php echo base_url(); ?>blocksamiti/get_booth_details',
                type: 'POST',
                ...
                error: function(xhr, status, error) {
                    console.error('AJAX Error loading booth details:', status, error);
                }
            });
        }
    });
