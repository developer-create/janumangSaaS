...
class Role_model extends CI_Model
{
    ...
    private function getRoleAccessMatrixQuery($roleId)
    {
        ...
        $this->db->where('roleId', $roleId);
        $this->db->where('isDeleted', 0); // Only get non-deleted records
        $query = $this->db->get();
    ...
    function updateAccessMatrix($roleId, $accessMatrix)
    {
        ...
        $this->db->where('roleId', $roleId);
        $this->db->where('isDeleted', 0);
        $existing = $this->db->get('tbl_access_matrix')->row();
        ...
        if ($existing) {
            ...
            $accessMatrix['roleId'] = $roleId;
            $accessMatrix['createdBy'] = isset($accessMatrix['updatedBy']) ? $accessMatrix['updatedBy'] : 1;
            $accessMatrix['createdDtm'] = isset($accessMatrix['updatedDtm']) ? $accessMatrix['updatedDtm'] : date('Y-m-d H:i:s');
            $accessMatrix['isDeleted'] = 0;
            $this->db->insert('tbl_access_matrix', $accessMatrix);
            return $this->db->affected_rows();
        }
    }
